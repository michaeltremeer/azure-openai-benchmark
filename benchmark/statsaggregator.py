    ...
    def aggregate_request(self, stats: RequestStats):
        ...
        with self.lock:
            ...
            if stats.response_status_code != 200:
                ...
                self.request_timestamps._append(
                    stats.request_start_time, stats.request_start_time
                )
                self.response_latencies._append(
                    stats.request_start_time,
                    stats.response_time - stats.request_start_time,
                )
                self.first_token_latencies._append(
                    stats.request_start_time,
                    stats.first_token_time - stats.request_start_time,
                )
                ...
                self.context_tokens._append(
                    stats.request_start_time, stats.context_tokens
                )
                self.generated_tokens._append(
                    stats.request_start_time, stats.generated_tokens
                )
            if stats.deployment_utilization is not None:
                self.utilizations._append(
                    stats.request_start_time, stats.deployment_utilization
                )
    ...
    def _dump(self):
        with self.lock:
            ...
            e2e_latency_avg = (
                ...
                if self.request_latency._len() > 0
                else "n/a"
            )
            e2e_latency_95th = (
                round(np.percentile(self.request_latency._values(), 95), 3)
                if self.request_latency._len() > 1
                else "n/a"
            )
            context_per_minute = (
                round(
                    60.0 * np.sum(self.context_tokens._values()) / self.window_duration,
                    0,
                )
                if self.context_tokens._len() > 0
                else "n/a"
            )
            gen_per_minute = (
                round(
                    60.0
                    * np.sum(self.generated_tokens._values())
                    / self.window_duration,
                    0,
                )
                if self.generated_tokens._len() > 0
                else "n/a"
            )
            tokens_per_minute = 0
            ...
            ttft_avg = (
                ...
                if self.first_token_latencies._len() > 0
                else "n/a"
            )
            ttft_95th = (
                round(np.percentile(self.first_token_latencies._values(), 95), 3)
                if self.first_token_latencies._len() > 1
                else "n/a"
            )
            tbt_avg = (
                round(np.average(self.token_latencies._values()), 3)
                if self.token_latencies._len() > 0
                else "n/a"
            )
            tbt_95th = (
                round(np.percentile(self.token_latencies._values(), 95), 3)
                if self.token_latencies._len() > 1
                else "n/a"
            )
            util_avg = (
                f"{round(np.average(self.utilizations._values()), 1)}%"
                if self.utilizations._len() > 0
                else "n/a"
            )
            util_95th = (
                f"{round(np.percentile(self.utilizations._values(), 95), 1)}%"
                if self.utilizations._len() > 1
                else "n/a"
            )
            rpm = (
                round(60.0 * self.request_timestamps._len() / self.window_duration, 1)
                if self.request_timestamps._len() > 0
                else "n/a"
            )
            # Handle the 1x extra processing_request due to next request being queued
            processing_requests_count = min(
                self.clients, self.processing_requests_count
            )
            if self.json_output:
                j = {
